...
void MyOctant::Init(void)
{
	...
	maxEntityCount = iEntityCount;
	m_EntityList = new MyEntity*[maxEntityCount];
...
void Simplex::MyOctant::AddToEntityList(MyEntity * e )
{
	
	if (entityCount + 1 >= maxEntityCount) 
	{
		entityCount++;
		MyEntity** temp = m_EntityList;
		maxEntityCount *= 2;
		m_EntityList = new MyEntity*[maxEntityCount];
...
MyOctant::MyOctant(uint a_nMaxLevel, uint a_nIdealEntityCount) 
{
	...
	m_uOctantCount++;
	m_pRigidBody->AddToRenderList();//adds another octant to the count 
...
MyOctant::MyOctant(vector3 a_v3Center, float a_fSize)
{
	isRoot = false;
	Init();//initialize the singleton entity manager and mesh manager 
	std::vector<vector3> minMax = std::vector<vector3>();
	minMax.push_back(a_v3Center+(a_fSize)*1.1f);
	minMax.push_back(a_v3Center-(a_fSize)*1.1f);
	...
	//adds another octant to the count 
...
void MyOctant::Add(MyEntity* ent) 
{
	...
	else 
	{
		...
		if (entityCount >= m_uIdealEntityCount && t < m_uMaxLevel)
		{
			Subdivide();//make more octants
