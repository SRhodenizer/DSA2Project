...
uint Octree::m_uOctantCount;
uint Octree::m_uMaxLevel;
uint Octree::m_uIdealEntityCount;
...
//returns the Octree size
float Octree::GetSize(void)
{
	return m_fSize;
}
...
bool Octree::IsColliding(MyEntity e)
{
...
void Octree::Display(uint a_nIndex, vector3 a_v3Color)
{
...
void Octree::DisplayLeafs(vector3 a_v3Color)
{
...
void Octree::Subdivide(void)
{
	if (IsLeaf()) //if the octant is a leaf 
	{
		...
		for (int i = 0; i < 8; i++)
		{
			...
			m_pChild[i] = new Octree(centerPoints[i], (m_fSize / 2.0f));
...
{
	for each(Octree* child in m_pChild)//for all children in the octree 
	{
...
void Octree::ConstructTree(uint a_nMaxLevel)
{

}

void Octree::AssignIDtoEntity(void)
{
...
void Octree::ConstructList(void)
{

}

//adds an entity to the entity list
void Octree::Add(MyEntity ent)
{
	if (!IsLeaf())
	{
		AddToChildren(ent);
...
void Octree::AddToChildren(MyEntity ent)
{
	for each(Octree* child in m_pChild)//for all children in the octree 
	{
		if (child->IsLeaf() == true)//if there are no children
...
void Simplex::Octree::CheckCollisions(void)
{
	if (IsLeaf())
	{
